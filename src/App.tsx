import { Instagram, Palette, Globe } from 'lucide-react';

function App() {
  // Configuração do WhatsApp
  // 1. Número no formato internacional (55 + DDD + Número)
  const whatsAppNumber = "5513974276219";
  
  // 2. Mensagem solicitada, codificada para ser usada na URL
  const rawMessage = "Olá! Gostaria de saber sobre seus serviços de design.";
  const whatsappMessage = encodeURIComponent(rawMessage);
  
  // 3. URL completa do WhatsApp com a mensagem
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-700 to-slate-800">
      ...
      <header className="bg-white shadow-sm">
        <div className="max-w-7xl mx-auto px-6 py-5 flex items-center justify-between lg:justify-center lg:relative">
          ...
          {/* Botão "Contato" com o redirecionamento para o WhatsApp */}
          <a 
            href={whatsAppLink} 
            target="_blank" // Abre o link em uma nova aba
            rel="noopener noreferrer" // Boa prática de segurança
            className="px-4 py-2 lg:px-6 lg:py-2.5 bg-slate-900 text-white rounded-lg text-sm lg:text-base font-medium hover:bg-slate-800 transition-colors duration-200 text-center lg:absolute lg:right-6"
          >
            Contato
          </a>
        </div>
